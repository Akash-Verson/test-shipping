                ...
                <div class='sb-header-left sb-left sb-table'>
                    ...
                    <div class='sb-header-item sb-table-cell'>
                        <div id='sb-header-text' class='e-sb-header-text'>
                            <span class='sb-header-text-left'>Essential Studio<sup>®</sup> Spreadsheet Editor SDK for </span>
            ...
            <div id='sb-popup-section' class='sb-popups'>
                <div id='sb-switcher-popup' role='navigation' class='sb-switch-popup'>
                    <ul id='switch-sb' role="list">
                        ...
                        <li role="listiem">
                            <a id='javascript-es5'>JavaScript (ES5)</a>
                        </li>
                        <li role="listiem">
                            <a id='javascript'>JavaScript</a>
                        </li>
                        <li role="listiem">
                            <a id='asp-net-core'>ASP.NET Core</a>
                        </li>
                        <li role="listiem">
                            <a id='asp-net-mvc'>ASP.NET MVC</a>
                        </li>
                        <li role="listiem">
                        ...
                        <div class="sb-footer">
                            <div class="sb-footer-left">
                                <div class="sb-footer-links">
                                    <a href="https://help.syncfusion.com/document-processing/introduction" target="_blank"
...
const sbArray: string[] = ['angular', 'nextjs', 'react', 'javascript-es5', 'asp-net-core', 'asp-net-mvc', 'javascript', 'blazor'];
...
// Banner defaults cache
let defaultBanner = { text: '', href: '', bulletHtml: '' };

// Detect current component from URL hash
const getComponentNameFromUrl = (): string | null => {
    const hash: string = (window.location.hash || '').replace(/^#\/?/, '');
    const parts: string[] = hash.split('/').filter(Boolean);
    const candidate: string = (parts[1] || parts[0] || '').toLowerCase();
...
// Update banner header and CTA based on active component
const updateBannerForComponent = (): void => {
    const bannerHead: HTMLElement | null = select('#banner-head', rootEle) as HTMLElement;
    const bannerCta: HTMLAnchorElement | null = (select('.banner-wrapper a[aria-label*="TRY IT FOR FREE"]', rootEle) as HTMLAnchorElement)
        || (select('.banner-wrapper a', rootEle) as HTMLAnchorElement);
    const firstBullet: HTMLElement | null = select('.banner-wrapper .banner-cnt-pt .banner-cnt-text', rootEle) as HTMLElement;
    if (!bannerHead || !bannerCta) return;

    // Initialize defaults once
    if (!defaultBanner.text) defaultBanner.text = bannerHead.innerHTML;
    if (!defaultBanner.href) defaultBanner.href = bannerCta.getAttribute('href') || '';
    if (!defaultBanner.bulletHtml && firstBullet) defaultBanner.bulletHtml = firstBullet.innerHTML;

    const component = getComponentNameFromUrl();
    if (component === 'pdf') {
        // Transform banner content for PDF component
        bannerHead.textContent = 'Transform your Vue apps today with Syncfusion ';
        const sup = document.createElement('sup');
        sup.textContent = '®';
        bannerHead.appendChild(sup);
        bannerHead.appendChild(document.createTextNode(' JavaScript PDF Library'));

        // Point CTA to target PDF page (adjustable if needed)
        const pdfCta = 'https://www.syncfusion.com/downloads/document-sdk?tag=es-seo-document-sdk-download-trial';
        bannerCta.href = pdfCta;

        // Update first bullet point text (no link) about PDF processing
        if (firstBullet) {
            firstBullet.textContent = 'High performance PDF document processing library';
        }
    } else {
        // Restore defaults for non-PDF components
        bannerHead.innerHTML = defaultBanner.text;
        if (defaultBanner.href) bannerCta.href = defaultBanner.href;
        if (firstBullet && defaultBanner.bulletHtml) firstBullet.innerHTML = defaultBanner.bulletHtml;
    }
};
...
const breadCrumbUpdate = (controlName: string, category: string, sampleName: string) => {
    ...
    if (controlName === 'spreadsheet' || controlName === 'PDF Viewer') {
        const hashParts: string[] = location.hash.split('/');
        const themeTag: string   = hashParts[1];
        const controlTag: string = hashParts[2];
        const sampleTag: string  = hashParts[3];
        const components: any = { 'spreadsheet': 'spreadsheet-editor', 'PDF Viewer': 'pdf-viewer', 'document-editor': 'docx-editor', 'PDF': 'pdf' };
        const canonicalUrl: string = `https://document.syncfusion.com/demos/${components[controlName]}/vue/#/${themeTag}/${controlTag}/${sampleTag}`;
...
const SbLink = (): void => {
    ...
    let sampleName: string = sample.split('/')[1];
    let componentName: string;
    if (sampleName === 'spreadsheet') {
        componentName = 'spreadsheet-editor';
    } else if (sampleName === 'pdfviewer') {
        componentName = 'pdf-viewer';
    } else if (sampleName === 'document-editor') {
        componentName = 'docx-editor';
    } else if (sampleName === 'pdf') {
        componentName = 'pdf';
    } 
    for (let sb of sbArray) {
        ...
        if (componentName === 'pdf' && ['asp-net-core', 'angular', 'react', 'javascript-es5', 'javascript'].indexOf(sb) === -1) {
            if (ele) {
                ele.parentElement.style.display = 'none';
            }
            continue;
        }
        if (sb === 'asp-net-core' || sb === 'asp-net-mvc') {
            if (sampleName === 'spreadsheet') {
                ele.href = 'https://document.syncfusion.com/demos/spreadsheet-editor/' + sb + '/spreadsheet/defaultfunctionalities';
            } else if (sampleName === 'pdfviewer') {
                ele.href = 'https://document.syncfusion.com/demos/pdf-viewer/' + sb + '/pdfviewer/default';
            } else if (sampleName === 'document-editor') {
                ele.href = 'https://document.syncfusion.com/demos/docx-editor/' + sb + '/documenteditor/' + ((sb === 'asp-net-core') ? 'default' : 'defaultfunctionalities');
            } else if (sampleName === 'pdf') {
                ele.href = 'https://document.syncfusion.com/demos/javascriptpdf/default';
            }
        } else if (sb === 'blazor') {
            if (sampleName === 'spreadsheet') {
                ele.href = 'https://document.syncfusion.com/demos/spreadsheet-editor/blazor-server/spreadsheet/overview';
            } else if (sampleName === 'pdfviewer') {
                ele.href = 'https://document.syncfusion.com/demos/pdf-viewer/blazor-server/pdf-viewer/default-functionalities';
            } else if (sampleName === 'document-editor') {
                ele.href = 'https://document.syncfusion.com/demos/docx-editor/blazor-server/document-editor/default-functionalities';
            }
        } 
        else {
            ele.href = 'https://document.syncfusion.com/demos/' + componentName + '/' + (sb + (sb === 'nextjs' ? '/' : '/#/')) + sample + (sb === ('javascript' || 'javascript-es5') ? '.html' : ''); 
        }
    }
};
...
const onOpenNewButtonClick = (arg: any): void => {
    ...
    let baseHref: string = location.href.split('#')[0];
    const suffix: string = 'index.html';
    ...
    (select('#openNew') as HTMLFormElement).href = baseHref + samplePath[2] + '/' + samplePath[3].replace('.html', '/');
};
...
const updatesourceTab = (): void => {
    ...
    ajaxvue.send().then((value: Object): void => {
        ...
        .catch((reason: any): void => {
            // Ensure overlay is removed on error so UI isn't blocked by loader
            try { removeOverlay(); } catch (e) { /* ignore */ }
            console.error('Failed to load sample source:', reason);
        });
};
